...
import { getProxiedImageUrl } from "@/app/utils/imageUrlUtils";

// Create a ref that persists across component mounts to track script loading
let scriptLoadedGlobal = false;

// Timeout for slow URL fetch (in milliseconds)
const SLOW_URL_TIMEOUT = 8000;

/**
 * Universal image URL processor - works with any image URL
 * R2 presigned URLs, Google Drive, local files, S3, etc. - all work the same
 * @param {string} url - Any image URL
 * @returns {string} - Processed URL (R2/direct URLs as-is, Drive URLs proxied)
 */
const processImageUrl = (url) => {
  ...
  // All other URLs (local, S3, CloudFront, Cloudflare Images, etc.) - direct access
  return url;
};

/**
 * Universal image preloader with timeout and fallback
 * Works with any image source: R2, Google Drive, S3, local files, etc.
 * @param {string} primaryUrl - The primary image URL (will be tried first)
 * @param {string} fallbackUrl - The fallback image URL (used if primary fails)
 * @param {number} timeout - Timeout in milliseconds (default: SLOW_URL_TIMEOUT)
 * @returns {Promise<string>} - The URL that should be used
 */
const fetchImageWithFallback = (
  primaryUrl,
  fallbackUrl,
  timeout = SLOW_URL_TIMEOUT,
) => {
  return new Promise((resolve) => {
    ...
    // If no primary URL, return fallback immediately
    if (!primaryUrl) {
      resolve(fallbackUrl);
      return;
    }

    // If no fallback URL, use primary
    if (!fallbackUrl) {
      resolve(primaryUrl);
      return;
    }
    ...
    // Success - primary URL loaded successfully
    img.onload = () => {
      console.log("Primary URL loaded successfully");
      resolveUrl(primaryUrl);
    };

    // Error - primary URL failed, use fallback
    img.onerror = () => {
      console.warn("Primary URL failed to load, using fallback");
      resolveUrl(fallbackUrl);
    };

    // Set timeout - if primary URL takes too long, use fallback
    timeoutId = setTimeout(() => {
      console.warn("Primary URL timed out, using fallback");
      resolveUrl(fallbackUrl);
    }, timeout);

    // Start loading the primary URL
    img.src = primaryUrl;
...
/**
 * Universal image URL selector - works with any image source
 * For local development: Uses proxy to avoid CORS issues
 * For production: Uses R2 URLs directly (faster)
...
const getImageUrl = (properties, isHD) => {
  ...
  // Production or when featureId is not available: Use R2 URL directly
...
const PannellumViewer = ({
  ...
  const generatePoi = async () => {
    ...
    // Get the R2 URL (primary) and direct fallback URL
    const r2Url = isHDMode
      ? selectedImage.properties.r2Url_High
      : selectedImage.properties.r2Url_Comp;

    const fallbackUrl = isHDMode
      ? selectedImage.properties.imageUrl_High ||
        selectedImage.properties.imageUrl
      : selectedImage.properties.imageUrl_Comp ||
        selectedImage.properties.imageUrl;

    // Use R2 URL directly, fallback to direct URL if needed
    let imageUrl;
    if (r2Url && fallbackUrl && r2Url !== fallbackUrl) {
      // Try R2 URL first, fallback to direct URL on failure
      imageUrl = await fetchImageWithFallback(r2Url, fallbackUrl);
    } else {
      // Use whichever URL is available
      imageUrl = r2Url || fallbackUrl || "";
    }
